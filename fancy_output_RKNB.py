...
filenames = [
#    'ord_cond_search_methB_seed16.mw',
#    'ord_cond_search_methB_seed17.mw',
    'ord_cond_search_methB_seed18.mw']
...
def calc_dist(nu1, nu22) :
    dist = 0.0
    for i in range(len(nu1)) :
        dist += fabs(float(nu1[i].replace(' ','')) - float(nu2[i].replace(' ','')))
    return dist
...
subs2 = '{a1 = c[2], a2 = c[3]-c[2], a3 = c[4]-c[3], a4 = c[5]-c[4], a5 = 1-c[5], b1 = b[1], b2 = b[2], b3 = b[3], b4 = b[4], b5 = b[5], b6 = b[6]}'
...
for sol in cl_sols :
    ...
    print('assign(evalf(subs(%s, %s))):' % (subs2, expcoe)) # unassign
                                                            # coe0, coe1...
    print('assign(%s):' % (subs2)) # unassign a1..a5, b1..b6
    ...
    print('abs(Im(b5))<10^(-10) and ', end='')
    print('abs(Im(b6))<10^(-10) ) then ', end='')
    print(r'printf("REAL\n"): ', end='')
